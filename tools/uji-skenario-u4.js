...
async function jalankanU4(options = {}) {
  ...
  console.log(`\n==============================================================`);
  console.log(`=== SKENARIO U4: Dead Letter (X01 Cacat) & V01 Valid [${runId}] ===`);
  console.log(`==============================================================`);
  ...
  ch.publish(spec.exchange, spec.routingKey, Buffer.from(JSON.stringify(invalidEventX01)), {
    persistent: true,
  ...
  console.log('-> X01 dan V01 terkirim. Menunggu verifikasi worker (3 detik)...');
  await delay(3000);

  const totalLedgerAkhirRes = await pool.query("SELECT count(*)::int AS count FROM ledger_penerimaan");
  const saldoAkhirRes = await pool.query("SELECT saldo FROM stok_barang WHERE sku = 'SKU-001'");
  const rejectedCountRes = await pool.query("SELECT count(*)::int AS count FROM rejected_events WHERE event_id = $1", [`${runId}-X01`]);
  const v01RecordedRes = await pool.query("SELECT count(*)::int AS count FROM ledger_penerimaan WHERE event_id = $1", [`${runId}-V01`]);
  ...
  console.log(`[U4 Hasil] Total Ledger Akhir: ${totalLedgerAkhir} (Ledger bertambah ${totalLedgerAkhir - ledgerSebelum}, V01 tercatat: ${v01Recorded})`);
  console.log(`[U4 Hasil] Saldo Akhir: ${saldoAkhir} (Saldo bertambah ${saldoAkhir - saldoSebelum})`);
  console.log(`[U4 Hasil] X01 tercatat di tabel rejected_events: ${rejectedCount}`);
  ...
  const pass = rejectedCount >= 1 && v01Recorded &&
    (options.standalone ? ((totalLedgerAkhir === ledgerSebelum + 1) && (saldoAkhir === saldoSebelum + 10)) : (totalLedgerAkhir === 26 && saldoAkhir === 190));
  ...
  writeFileSync(`${evidenceDir}/${runId}-u4.json`, JSON.stringify(result, null, 2));
