...
async function main() {
  const runId = process.argv[2] || `run${Date.now().toString().slice(-4)}`;
  ...
  console.log(`\n##############################################################`);
  console.log(`### MEMULAI RANGKAIAN LENGKAP PENGUJIAN KASUS A10 [${runId}] ###`);
  console.log(`##############################################################`);
  ...
  const hasilU3 = await jalankanU3({ ...sharedContext, u1Events: hasilU1.events });
  ...
  console.log(`U1 (Normal 20 Event)       : ${hasilU1.pass ? 'LULUS [PASS]' : 'GAGAL [FAIL]'}`);
  console.log(`U2 (Recovery 5 Event)      : ${hasilU2.pass ? 'LULUS [PASS]' : 'GAGAL [FAIL]'}`);
  console.log(`U3 (Replay 5 Event Idemp)  : ${hasilU3.pass ? 'LULUS [PASS]' : 'GAGAL [FAIL]'}`);
  console.log(`U4 (Dead Letter & V01)     : ${hasilU4.pass ? 'LULUS [PASS]' : 'GAGAL [FAIL]'}`);
  const allPass = hasilU1.pass && hasilU2.pass && hasilU3.pass && hasilU4.pass;
  console.log(`Status Keseluruhan          : ${allPass ? 'SEMUA SKENARIO LULUS 100%' : 'ADA SKENARIO GAGAL'}`);
  console.log('==============================================================');
